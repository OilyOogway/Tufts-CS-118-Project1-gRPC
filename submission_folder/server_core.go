...
// Implementation of the Connect method defined in .proto file.
// Change username from Registration to an AuthToken object.
// Tokens unique to user if logged in helps make sure the connect is rejected
// if user already logged in. Also creates an entry in s.AuthToUserTable
// and s.Inboxes.
...
func (s Server) Send(ctx context.Context, msg *ChatMessage) (*Success, error) {
	...
	if inbox, exists := s.Inboxes[msg.User]; exists {
		...
		// send the message to the target user's inbox. Have default case for
		// error catching
		select {
...
func (s Server) Fetch(ctx context.Context, _ *Empty) (*ChatMessages, error) {
	...
	for i := 0; i < BATCH_SIZE; i++ {
		...
		case msg, ok := <-inbox:
			if !ok {
				// channel is closed, return all messages we have
				return &ChatMessages{Messages: messages}, nil
			}
			messages = append(messages, msg)
		default:
			// no more messages to read, return what we have
			return &ChatMessages{Messages: messages}, nil
		}
	}
